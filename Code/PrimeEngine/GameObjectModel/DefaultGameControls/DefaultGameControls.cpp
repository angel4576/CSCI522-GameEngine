...
void DefaultGameControls::handleKeyboardDebugInputEvents(Event *pEvt)
{
	...
	{
		Handle h("EVENT", sizeof(Event_ADD_WIND));	
		Event_ADD_WIND* addWindEvt = new(h) Event_ADD_WIND;

		m_pQueueManager->add(h, QT_GENERAL);
	}
	else if (Event_KEY_P_HELD::GetClassId() == pEvt->getClassId())
	{
		Handle h("EVENT", sizeof(Event_REDUCE_WIND));
		Event_REDUCE_WIND* reduceWindEvt = new(h) Event_REDUCE_WIND;
