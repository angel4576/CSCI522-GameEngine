...
Vector3 MuliplyMatrixByVector(Matrix4x4& mat, Vector3& vec)
{
	Vector4 res;
	Vector4 homoPos = Vector4(vec.m_x, vec.m_y, vec.m_z, 1);

	res.m_x = mat.m[0][0] * homoPos.m_x + mat.m[0][1] * homoPos.m_y + mat.m[0][2] * homoPos.m_z + mat.m[0][3] * homoPos.m_w;
	res.m_y = mat.m[1][0] * homoPos.m_x + mat.m[1][1] * homoPos.m_y + mat.m[1][2] * homoPos.m_z + mat.m[1][3] * homoPos.m_w;
	res.m_z = mat.m[2][0] * homoPos.m_x + mat.m[2][1] * homoPos.m_y + mat.m[2][2] * homoPos.m_z + mat.m[2][3] * homoPos.m_w;
	res.m_w = mat.m[3][0] * homoPos.m_x + mat.m[3][1] * homoPos.m_y + mat.m[3][2] * homoPos.m_z + mat.m[3][3] * homoPos.m_w;
	
	return Vector3(res.m_x / res.m_w, res.m_y / res.m_w, res.m_z / res.m_w);
	//return Vector3(res.m_x, res.m_y, res.m_z);
}
...
void SingleHandler_DRAW::do_GATHER_DRAWCALLS(Events::Event *pEvt)
{
	...
	if (false && pMeshCaller->m_performBoundingVolumeCulling)
    {
		...
		PE::Components::CameraSceneNode* pcam = PE::Components::CameraManager::Instance()->getActiveCamera()->getCamSceneNode();
        ...
        for (int iInst = 0; iInst < pMeshCaller->m_instances.m_size; ++iInst)
        {
			...
			// Parent scene node of the mesh instance
			SceneNode* pSN = pInst->getFirstParentByTypePtr<PE::Components::SceneNode>();

			Vector3 min_bbx = pMeshCaller->m_bbx[0];
			Vector3 max_bbx = pMeshCaller->m_bbx[1];

			min_bbx = pSN->m_worldTransform * min_bbx;
			max_bbx = pSN->m_worldTransform * max_bbx;

			// bool isInFrustrum = pcam->isWithinPlane(pcam->m_frustum.m_farFace, min_bbx, max_bbx);
			bool isInFrustrum = pcam->isAABBInFrustum(min_bbx, max_bbx);
