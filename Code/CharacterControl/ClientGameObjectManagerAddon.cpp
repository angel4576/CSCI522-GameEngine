...
void ClientGameObjectManagerAddon::do_CreateParticleSystem(PE::Events::Event* pEvt)
{
...
void ClientGameObjectManagerAddon::createTank(int index, int &threadOwnershipMask)
{
	...
	pSN->addComponent(hParticleSystem);*/
}

void ClientGameObjectManagerAddon::createParticleSystem(Vector3 position, int& threadOwnershipMask)
{
	// M1: add particle system
	PE::Handle hParticleSystem("ParticleSystem", sizeof(ParticleSystem));
	ParticleSystem* pParticleSystem = new(hParticleSystem) ParticleSystem(*m_pContext, m_arena, hParticleSystem
		, 2.f);
	pParticleSystem->addDefaultComponents();

	addComponent(hParticleSystem);
}
