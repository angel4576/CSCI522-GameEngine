...
int ClientGame::runGameFrame()
{
    ...
    while (!gqh.getObject<Events::EventQueue>()->empty())
    {
        ...
        if (Event_UPDATE::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			// ParticleSystem::Instance()->handleEvent(pGeneralEvt);
        }
        else if (Event_CALCULATE_TRANSFORMATIONS::GetClassId() == pGeneralEvt->getClassId())
        {
            ...
            // Push DRAW event into message queue because camera has updated transformations
            {
				...
				// Frustum copy
				drawEvt->m_frustumLeft = pcam->m_leftPlane;
				drawEvt->m_frustumRight = pcam->m_rightPlane;
				drawEvt->m_frustumBottom = pcam->m_bottomPlane;
				drawEvt->m_frustumTop = pcam->m_topPlane;
				drawEvt->m_frustumNear = pcam->m_nearPlane;
				drawEvt->m_frustumFar = pcam->m_farPlane;
				
				// drawEvt->m_camFrustum = pcam->m_frustum;
			...
			|| Event_GATHER_DRAWCALLS_Z_ONLY::GetClassId() == pGeneralEvt->getClassId())
        {
			...
			if (!zOnly)
			{
				...
				// A4: Physics Manager handles pre render events
				PhysicsManager::Instance()->handleEvent(&preRenderEvt);
